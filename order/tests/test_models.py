...
def create_registered_customer():
    user = User.objects.create_user(email="user@example.com", username="testuser")
...
class TestOrderModelsGuest(TestCase):
    ...
    def test_create_order_item_guest_no_variants(self):
        ...
        self.assertEqual(str(order_item), f"{product.name} #{order_item.quantity}")
    ...
    def test_order_item_properties_guest_with_variation(self):
        ...
        first_variant = ProductVariant.objects.filter(title="Test Variant 1")[0]
        order = Order.objects.create(customer=customer)
        order_item = OrderItem.objects.create(
    ...
    def test_order_properties_no_coupon_with_variants_guest(self):
        ...
        customer = create_guest_customer()
        product = create_test_product_with_variants()
        product_variant_1 = ProductVariant.objects.filter(title="Test Variant 1")[0]
        product_variant_2 = ProductVariant.objects.filter(title="Test Variant 2")[0]
        order = Order.objects.create(customer=customer)
    ...
    def test_order_properties_with_percent_coupon_guest(self):
        ...
        self.assertEqual(order.get_cart_items, 3)
        self.assertEqual(order.get_coupon_value, Decimal(str(13 * 0.2)))
        self.assertEqual(order.get_cart_total, Decimal(str(13 * 0.8)))
...
class TestOrderModelsRegisteredUser(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.customer = create_registered_customer()
    ...
    def test_create_order_by_customer(self):
        ...
        self.assertEqual(str(order), f"{order.transaction_id} by {self.customer}")
    ...
    def test_create_order_item_by_customer_no_variants(self):
        ...
        self.assertEqual(order_item.product, product)
        self.assertEqual(order_item.order, order)
        self.assertEqual(order_item.order.customer, self.customer)
...
class TestOrderModels(TestCase):
    ...
    def test_create_coupon_percent(self):
        ...
        self.assertEqual(str(coupon), "TestCodePercent type: Percent value: 20")
    ...
    def test_create_shipping_address(self):
        ...
        self.assertEqual(
            str(shipping),
            f"{shipping.first_name} {shipping.last_name} {shipping.address_1}",
        )
